...
- Session 19: Voice Assistant & Voice PTT (Added microphone speech-to-text (STT) inside the AI chat input. Implemented a global PTT voice assistant triggered by holding/releasing Ctrl+Alt+V with premium listening, transcribing, and thinking overlays, custom brevity prompt injection, and a wireframed floating Voice Response overlay card with progress countdown and auto-dismiss).
