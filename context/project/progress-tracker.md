...
- Session 23: Media Compatibility (Resolve media capture failures on laptops lacking full DXGI Desktop Duplication support and reduce SMTC polling log warning spam.)
...
- Session 23: Media Compatibility (Implemented GDI fallback for screenshot captures when DXGI is unsupported, integrated FFmpeg gdigrab and software libx264 encoding for recordings and replays, added UI selectors for Capture Mode and Video Encoder, checked DXGI support on application boot with toast notifications and automatic configuration fallbacks, demoted SMTC polling session warnings to debug level, applied layout fixes to settings modal hotkey records, hardware temp tooltips and audio mixer height constraints, created a cross-platform mock widget test suite, and bumped version to 1.2.2 to verify key rotation updater compatibility).
