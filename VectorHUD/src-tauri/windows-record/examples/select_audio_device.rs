...
use std::{env, io};
use windows_record::{
    enumerate_audio_input_devices, AudioInputDevice, Recorder, RecorderConfig, Result,
};
...
fn main() -> Result<()> {
    ...
    let selected_device: Option<&AudioInputDevice> =
        if device_idx > 0 && device_idx <= devices.len() {
    ...
    info!(
        "Starting recording with{} microphone device.",
