...
/// Structure holding information about an audio output device
#[derive(Debug, Clone)]
...
pub fn enumerate_audio_output_devices() -> Result<Vec<AudioOutputDevice>> {
    unsafe {
        ...
        if let Err(e) = coinit_result {
            if e.code() != CO_E_ALREADYINITIALIZED
                && e.code() != windows::Win32::Foundation::RPC_E_CHANGED_MODE.into()
            {
                return Err(e);
            }
        }
        ...
        // Enumerate all active audio playback (render) devices
        ...
        for i in 0..count {
            if let Ok(device) = collection.Item(i) {
                ...
                if let Ok(id_ptr) = device.GetId() {
                    ...
                    let device_id = OsString::from_wide(id_wide_slice)
                        .to_string_lossy()
                        .to_string();
                    ...
                    if let Ok(props) = device.OpenPropertyStore(STGM_READ) {
                        if let Ok(prop_value) = props.GetValue(&PKEY_Device_FriendlyName) {
                            let name_wide_slice =
                                prop_value.Anonymous.Anonymous.Anonymous.pwszVal.as_wide();
                            let device_name = OsString::from_wide(name_wide_slice)
                                .to_string_lossy()
                                .to_string();
...
pub fn get_audio_input_device_by_name(device_name: Option<&str>) -> Result<String> {
    ...
    info!(
        "Specified audio input device '{}' not found, using default",
        name
    );
