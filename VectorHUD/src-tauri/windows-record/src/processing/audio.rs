...
impl AudioMixer {
    pub fn new(
        ...
        info!(
            "Creating AudioMixer: sample_rate={}, bits_per_sample={}, channels={}",
    ...
    pub fn add_system_audio(&mut self, sample: SendableSample) {
        trace!(
            "Adding system audio sample to queue (queue size: {})",
    ...
    pub fn add_microphone_audio(&mut self, sample: SendableSample) {
        trace!(
            "Adding microphone audio sample to queue (queue size: {})",
    ...
    pub unsafe fn process_next_sample(&mut self) -> Option<Result<Arc<IMFSample>>> {
        debug!(
            "Processing next sample - sys queue: {}, mic queue: {}",
            self.system_audio_queue.len(),
            self.microphone_queue.len()
        );
    ...
    unsafe fn apply_volume_to_sample(&self, sample: &IMFSample, volume: f32) -> Result<IMFSample> {
        ...
        let samples = std::slice::from_raw_parts(data as *const i16, length as usize / 2);

        // Get output as mutable slice of i16
        let output_samples =
            std::slice::from_raw_parts_mut(output_data as *mut i16, length as usize / 2);
    ...
    unsafe fn mix_samples(
        &self,
        sys_sample: &IMFSample,
        ...
        let result = (|| {
            ...
            debug!(
                "Buffer sizes - System: {} bytes, Microphone: {} bytes",
                sys_length, mic_length
            );
            ...
            // Mix the audio
            if let Err(e) =
                self.mix_pcm_audio(sys_data, mic_data, output_data, sys_length, mic_length)
            {
    ...
    unsafe fn mix_pcm_audio(
        ...
        debug!(
            "Mixing with system volume:{:.2} mic volume:{:.2}",
            self.system_volume, self.microphone_volume
        );

        // Access the audio data as 16-bit PCM
        let sys_samples =
            std::slice::from_raw_parts(sys_data as *const i16, sys_length as usize / 2);

        let mic_samples =
            std::slice::from_raw_parts(mic_data as *const i16, mic_length as usize / 2);
        ...
        let mix_len = std::cmp::min(
            sys_samples.len(),
            std::cmp::min(mic_samples.len(), output_samples.len()),
        );
        ...
        for i in 0..mix_len {
            ...
            let mixed_val = (sys_samples[i] as f32 * self.system_volume
                + mic_samples[i] as f32 * self.microphone_volume)
                as i32;
