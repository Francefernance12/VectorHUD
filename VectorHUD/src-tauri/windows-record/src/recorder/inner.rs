...
impl RecorderInner {
    ...
    pub fn init_with_exact_match(
        ...
        let mut input_width = config.input_width();
        let mut input_height = config.input_height();
        let mut output_width = config.output_width();
        let mut output_height = config.output_height();
        ...
        unsafe {
            ...
            // Just use the desktop window handle for any HWND requirements.
            let hwnd = windows::Win32::UI::WindowsAndMessaging::GetDesktopWindow();

            // Get the process ID of the desktop window for audio routing, or just 0
