...
impl TexturePool {
    ...
    pub fn new(
        ...
        for i in 0..capacity {
            ...
            #[cfg(debug_assertions)]
            debug!(
                "TexturePool: Created initial texture #{} at {:p}",
                i, &texture as *const _
            );
        ...
        info!(
            "TexturePool initialized with {} textures of {}x{}",
    ...
    pub fn acquire(&self) -> Result<ID3D11Texture2D> {
        ...
        let texture = if let Some(texture) = textures.pop() {
            ...
            #[cfg(debug_assertions)]
            {
                ...
                debug!(
                    "TexturePool: Created new texture at {:p}",
                    &texture as *const _
                );
            }
    ...
    pub fn release(&self, texture: ID3D11Texture2D) {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            if active % 30 == 0 || active <= 0 {
                // Log every 30 frames or when pool is empty
                debug!(
                    "TexturePool: After release - {} textures still active",
                    active
                );
            }
        }
