...
export function AudioHubWidget() {
  ...
  const [activeTab, setActiveTab] = useState<'mixer' | 'devices'>('mixer');
  ...
  const toggleMicTest = async () => {
    ...
    try {
      ...
      const match = (selectedAudioInput !== 'Default' && selectedAudioInput !== '')
        ? devices.find(d => d.kind === 'audioinput' && d.label && d.deviceId && (
            d.label.includes(selectedAudioInput) || selectedAudioInput.includes(d.label)
          ))
        : null;
      const constraints = {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 text-text-primary font-mono text-sm p-4 space-y-4 overflow-y-auto overflow-x-hidden w-full min-w-0 custom-scrollbar">
      
      {/* Media Player Section */}
      {currentMedia && (
        <div className="flex flex-col space-y-3 bg-black/40 border border-white/5 p-3 rounded-lg shrink-0">
      ...
      {/* Sleek HUD Tab Selector */}
      <div className="flex border border-white/5 bg-zinc-950/60 p-0.5 rounded-md font-mono text-[10px] w-full shrink-0">
        <button
          onClick={() => setActiveTab('mixer')}
          className={`flex-1 py-1.5 text-center uppercase tracking-widest transition-colors rounded-sm font-bold ${
            activeTab === 'mixer' 
              ? 'text-accent-green bg-accent-green/10 shadow-[inset_0_0_4px_rgba(74,246,38,0.2)]' 
      ...
      {/* Tab Content */}
      <div className="flex-grow flex-shrink min-h-0 w-full">
        {activeTab === 'mixer' ? (
          <div className="flex flex-col space-y-4 w-full">
            {/* Master Volume Section */}
            <div className="flex flex-col space-y-2 bg-black/30 border border-white/5 p-3 rounded-lg">
                  ...
                  <div key={session.process_id} className="flex flex-col space-y-1 group">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input 
                        ...
                        className="flex-grow h-1 bg-zinc-800 rounded-full appearance-none cursor-pointer accent-zinc-300 hover:accent-accent-blue transition-colors"
                      />
                    </div>
                  </div>
                ))}
          ...
          /* Devices Tab: Hardware Interfaces */
          <div className="flex flex-col space-y-4 bg-black/30 border border-white/5 p-3 rounded-lg min-w-0 w-full overflow-hidden">
            <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Hardware Interfaces</div>
            
            {/* Output Selector & Test */}
            <div className="flex flex-col space-y-1.5 min-w-0 w-full">
              <label className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide">Speaker / Output Device</label>
            ...
            <div className="flex flex-col space-y-2 min-w-0 w-full border-t border-white/5 pt-3">
              <label className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide">Microphone / Input Device</label>
              <div className="flex items-center gap-2 min-w-0 w-full">
                <select
